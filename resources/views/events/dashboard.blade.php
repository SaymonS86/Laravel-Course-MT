...
@section('content')
    ...
    <div class="flex flex-col items-center justify-center h-screen gap-y-32">
        ...
        <div class="title text-center py-12">
            <h1 class="text-4xl font-bold">Eventos que estou participando</h1>
        </div>
                    ...
                    <table class="mx-auto border-collapse border border-gray-300">
                        ...
                        <tbody>
                            @foreach ($eventsAsParticipants as $event)
                                <tr class="text-center">
                                    <th scope="row" class="p-6 border border-gray-300">{{ $loop->index + 1 }}</th>
                                    <td class="p-6 border border-gray-300">
                                        <a href="/events/{{ $event->id }}" class="text-blue-500 underline">{{ $event->title }}</a>
                                    </td>
                                    <td class="p-6 border border-gray-300">{{ count($event->users) }}</td>
                                    <td class="p-6 border border-gray-300">
                                        <a href="#" class="text-red-500 underline">Sair do evento</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
